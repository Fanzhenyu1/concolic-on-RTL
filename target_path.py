...
def path_generate(dict_CDFG_inout, target_node_list):
    ...
    for key, value in dict_CDFG_inout.items():           # 遍历所有节点
        ...
        for i in node_condition_in:
            if i.split('[')[0] in sub_node_action_out:      #@shangyihuigaidong
...
def main():
    ...
    print("目标节点块内路径：",dict_CDFG_inout[target_node]['block_path'])
    target_node_list = []
