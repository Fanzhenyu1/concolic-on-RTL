...
always @ ( i_tx_phy_state or TxValid_i or i_tx_phy_data_done or ( i_tx_phy_sft_done & !( i_tx_phy_sft_done_r) ) or i_tx_phy_append_eop_sync3 or i_rx_phy_fs_ce) begin 
    ...
    3'h3: begin  if (i_tx_phy_append_eop_sync3) begin $display("achieve node: 32,0,4");  i_tx_phy_next_state = 3'h4; end end 
    3'h4: begin  if (!(i_tx_phy_append_eop_sync3) && i_rx_phy_fs_ce) begin $display("achieve node: 32,0,5");  i_tx_phy_next_state = 3'h5; end end 
    3'h5: begin  if (i_rx_phy_fs_ce) begin $display("achieve node: 32,0,6");  $display("achieve node: 32,0,6,1"); i_tx_phy_next_state = 3'd0; end 
    end 
    endcase end
...
always @(i_rx_phy_dpll_state or i_rx_phy_rx_en or(i_rx_phy_rxd_r != i_rx_phy_rxd_s)) begin $display("achieve node: 54,0"); i_rx_phy_fs_ce_d = 1'b0; 
...
always @(posedge clk) begin 
    $display("achieve node: 61,1"); 
    if (i_rx_phy_rx_valid) begin 
        $display("achieve node: 61,1,1"); i_rx_phy_rx_valid_r <= 1'b1; 
    end else begin 
        $display("achieve node: 61,1,0"); 
        if (i_rx_phy_fs_ce) begin 
            $display("achieve node: 61,1,0,1"); i_rx_phy_rx_valid_r <= 1'b0; 
    end end end
